...
for P= 1:nit
   ...
   %% minimum circumscribing circle
    ...
    dist_pl=[];
    for i = 1:length(x) - 1
        d = [d; x(i) y(i) sqrt((graindata(1).Centroid(1,2) - x(i))^2 + (graindata(1).Centroid(1,2) - y(i))^2) i];% maximum distance from the centroied
    end
    ...
    for i=1: size(graindata(1).ConvexHull,1)
        for j=1: size(graindata(1).ConvexHull,1)
            dfc= sqrt((xc - graindata(1).ConvexHull(j,1))^2 + (yc - graindata(1).ConvexHull(j,2))^2);
            if dfc > rc
                xt= [xt; graindata(1).ConvexHull(j,1)];
                yt= [yt; graindata(1).ConvexHull(j,2)];
                dnc=[dnc; dfc];
            end
        end
        if size(xt,1) == 0
    ...
    points=[];
    lineLength = graindata(1).MajorAxisLength(1,1)*0.8;
    bp=[];
    thint=0.0017; % 0.1 degree upto 4th decimal place
    for angle = 0:thint:2*pi
        x1 = graindata(1).Centroid(1,1);
        y1 = graindata(1).Centroid(1,2);
    ...
    angle=[];
    for i=1:size(bp,1);
        x=graindata(1).Centroid(1,1)-bp(i,1);
        y=graindata(1).Centroid(1,2)-bp(i,2);
        r=sqrt(x^2+y^2);
...
%         title('Original Profile and Noise removed Profile');
    ...
    x_new= x_new+ graindata(1).Centroid(1,1);
    y_new= y_new+ graindata(1).Centroid(1,2);
    ...
    x_old= x_old+ graindata(1).Centroid(1,1);
    y_old= y_old+ graindata(1).Centroid(1,2);
...
%     hold on
   ...
   % to find perimeter ratio
    ...
    load('nm')
    Number_of_pixels= nm;
    perimeter_ratio=  d/graindata(1).Perimeter;
    % roundness
...
%Area sphericity
Sa = (graindata(1).Area)/(pi*radius_cir*radius_cir);
